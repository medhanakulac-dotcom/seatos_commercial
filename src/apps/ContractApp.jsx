...
const sbFetch=async(method,body)=>{
  try{
    const opts={method,headers:{"apikey":SUPABASE_KEY,"Authorization":"Bearer "+SUPABASE_KEY,"Content-Type":"application/json","Prefer":method==="PATCH"?"return=representation":""}};
    if(body)opts.body=JSON.stringify(body);
    const url=SUPABASE_URL+"/rest/v1/contract_builder_settings?id=eq.default";
    const r=await fetch(method==="GET"?url+"&select=*":url,opts);
...
const saveSettings=async(pricing,company)=>await sbFetch("PATCH",{pricing,company,updated_at:new Date().toISOString()});
